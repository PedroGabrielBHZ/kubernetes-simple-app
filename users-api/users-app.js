...
app.get("/get", async (req, res) => {
  console.log("Try and get me, motherfucker.");
  return res.status(422).json({ message: "Nothing to get here, fucker." });
});
...
app.post("/signup", async (req, res) => {
  ...
  try {
    ...
    console.log("AUTH ADDRESS:", process.env.AUTH_ADDRESS);
...
app.post("/login", async (req, res) => {
  ...
  if (response.status === 200) {
    console.log(`${email} logged in successfully using password ${password}`);
